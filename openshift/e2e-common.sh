...
export STRIMZI_INSTALLATION_CONFIG_TEMPLATE="test/config/100-strimzi-cluster-operator-0.23.0.yaml"
export STRIMZI_INSTALLATION_CONFIG="$(mktemp)"
export KAFKA_INSTALLATION_CONFIG="test/config/100-kafka-ephemeral-triple-2.8.0.yaml"
...
function install_consolidated_knative_kafka_channel(){
  ...
  cat ${RELEASE_YAML} \
  | sed "s/      brokers: REPLACE_WITH_CLUSTER_URL/      brokers: ${KAFKA_CLUSTER_URL}/" \
  | oc apply --filename -
